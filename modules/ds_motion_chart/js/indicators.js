...
function ds_mc_changeData() {
  // build up select clause of query string
  var colFragment = '';
  for (i=0; i<119; i++) {
    colFragment += 'col'+i+', ';
  }
  ...
  query.send(ds_mc_getData);
}

function ds_mc_getData(response) {
  var motionchart = new google.visualization.MotionChart(
     document.getElementById('visualization'));

  var table = response.getDataTable();

  var labels = ds_mc_getLabels();
  ...
  var options = {};
  options['state'] = Drupal.settings.ds_motion_chart['initial_state'];
  options['width'] = 800;
  options['height'] = 500;
